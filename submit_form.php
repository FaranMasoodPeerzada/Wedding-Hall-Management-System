...
$duplicate_check_sql = "SELECT * FROM bookings WHERE event_date='$event_date' AND event_start_time='$event_start_time' AND event_end_time='$event_end_time' AND hall_selection='$hall_selection'";
$duplicate_result = $conn->query($duplicate_check_sql);

if ($duplicate_result->num_rows > 0) {
    // Duplicate booking found
    echo "<script>alert('A booking with the same event date, start time, end time, and hall already exists. Please change the date or hall selection.'); window.history.back();</script>";
} else {
...
// Repeat this process for barbq, salad, naan and roti, sweet dishes, raita, drinks, and extras
    ...
    $sql = "INSERT INTO bookings (booking_date, event_date, event_start_time, event_end_time, event_purpose, customer_name, mobile_number, cnic, address, hall_selection, mains, rice, sweet_dishes, barbq, salad, naan_roti, raita, drinks, extras, ac_charges, stage_decoration, no_of_guests, dj, photography, service_charges, other_charges, total_amount, paid_amount, remaining_balance)
    VALUES ('$booking_date', '$event_date', '$event_start_time', '$event_end_time', '$event_purpose', '$customer_name', '$mobile_number', '$cnic', '$address', '$hall_selection', '$mains', '$rice', '$sweet_dishes', '$barbq', '$salad', '$naan_roti', '$raita', '$drinks', '$extras', '$ac_charges', '$stage_decoration', '$no_of_guests', '$dj', '$photography', '$service_charges', '$other_charges', '$total_amount', '$paid_amount', '$remaining_balance')";
