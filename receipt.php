...
if (isset($_GET['booking_id'])) {
    ...
    $booking_query = "SELECT b.*, h.price AS hall_charges
    FROM bookings b
    JOIN halls h ON b.hall_selection = h.hall_name
    ...
    if ($booking_result->num_rows > 0) {
        ...
        echo "<table border='2'>";
        echo "<tr><th colspan='2'>Booking Receipt id</th><td colspan='2'>{$booking_id}</td></tr>";
        echo "<tr><td colspan='2'>Booking Date:</td><td colspan='2'>{$booking_row['customer_name']}</td></tr>";
        echo "<tr><td colspan='2'>Booking Date:</td><td colspan='2'>{$booking_row['booking_date']}</td></tr>";
        echo "<tr><td colspan='2'>Event Date:</td><td colspan='2'>{$booking_row['event_date']}</td></tr>";
        echo "<tr><td colspan='2'>Event Time:</td><td colspan='2'>{$booking_row['event_start_time']} - {$booking_row['event_end_time']}</td></tr>";
        echo "<tr><td colspan='2'>Number of Guests:</td><td colspan='2'>{$booking_row['no_of_guests']}</td></tr>";
        echo "<tr><td colspan='2'>Hall:</td><td colspan='2'>{$booking_row['hall_selection']}</td></tr>";
        ...
        // Display additional 
        echo "<tr><td colspan='3'>Hall Charges:</td><td>{$booking_row['hall_charges']}</td></tr>";

        echo "<tr><td colspan='3'>AC Charges:</td><td>{$booking_row['ac_charges']}</td></tr>";
        echo "<tr><td  colspan='3'>Stage Decoration Charges:</td><td>{$booking_row['stage_decoration']}</td></tr>";
        echo "<tr><td  colspan='3'>Photography Charges:</td><td>{$booking_row['photography']}</td></tr>";
        echo "<tr><td  colspan='3'>DJ Charges:</td><td>{$booking_row['dj']}</td></tr>";
        echo "<tr><td  colspan='3'>Service Charges:</td><td>{$booking_row['service_charges']}</td></tr>";
        echo "<tr><td  colspan='3'>Other Charges:</td><td>{$booking_row['other_charges']}</td></tr>";

        // Calculate total amount
        $total_amount = $total_food_amount + $booking_row['ac_charges'] + $booking_row['stage_decoration'] + $booking_row['photography'] + $booking_row['dj'] + $booking_row['service_charges'] + $booking_row['other_charges'] + $booking_row['hall_charges'];

        // Display total amount
        echo "<tr><td colspan='2'></td><td>Total Amount:</td><td>$total_amount</td></tr>";

        // Input field for paid amount with dynamic calculation of remaining balance
        echo "<tr><td colspan='2'></td><td>Amount Paid:</td><td>";
        echo "<input type='text' id='paid_amount' name='paid_amount' value='{$booking_row['paid_amount']}' onchange='calculateRemainingBalance()'>";
        echo "</td></tr>";

        echo "<tr><td colspan='2'></td><td>Remaining Balance:</td><td><span id='remaining_balance'></span></td></tr>";
        echo "<tr><td colspan='4'></td></tr>";

        // Update button to update the amounts in the database
        echo "<tr><td colspan='2'><button onclick='updateAmounts()'>Add Advance</button></td><td colspan='2'><button onclick='window.print()'>Print</button></td></tr>";
